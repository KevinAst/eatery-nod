 ...
 *    **(appWillStart)**
 *
 *  - ?? inject our notify utility in the root DOM **(appWillStart)**
 *
 *  - ?? loads device resources (fonts, geo location), triggered by bootstrap
 *    action **(logic, action, reducer)**
 *
 *  - ?? monitors the device initialization progress, syncing the device status
 *    **(logic, action, reducer)**
 *
 *  - ?? disables downstream visuals until the device is ready - displaying a
 *    SplashScreen **(route)**
 *
 *  - ?? emits **ready** action (when appropriate by monitoring device status),
 *    triggering downstream app process **(logic)**:
 *    ```
